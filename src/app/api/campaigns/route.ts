...
export const mapCampaignDocumentToCampaign = (campaignDoc: Omit<Campaign, 'id'> & { _id: ObjectId }): Campaign => {
  return {
    ...
    title: campaignDoc.title || 'Untitled Campaign',
    brief: campaignDoc.brief || 'No brief provided.',
    ...
    agentDebates: (campaignDoc.agentDebates || []).map(ad => ({ ...ad, agentName: ad.agentName || 'Unknown Agent', message: ad.message || '', timestamp: ensureDate(ad.timestamp) || new Date() })),
